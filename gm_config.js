...
? false : (stored[i]||settings[i].default||'');
    }
 ...
 open: function() {
	...
	if(field.section[1]) frameBody.lastChild.appendChild(create('p', {className:'section_desc indent40',innerHTML:field.section[1]}));
	  ...
	  switch(field.type) {
	  case 'textarea':
	                fields.push(create('div', {title:field.title||'',kids:[
	  ...
	  case 'checkbox':
		...
		if(field.script) tmp.addEventListener('click', function(){eval(field.script);}, false);
		break;
	  default:   
	    fields.push(create('div', {title:field.title||'',kids:[
					    create('span', {textContent:label, className:'field_label'}),
